"""Extract and restructure license expiry section data."""
import json
import re
...
def extract_license_expiry_date(text):
    ...
    patterns = [
        ...
        r'تاریخ انقضا پروانه\s*:\s*([\d\u0660-\u0669\u06F0-\u06F9]{4}/[\d\u0660-\u0669\u06F0-\u06F9]{2}/[\d\u0660-\u0669\u06F0-\u06F9]{2})',  # Persian/Arabic-Indic digits
        r'انقضا\s*:\s*(\d{4}/\d{2}/\d{2})',
        r'پروانه\s*:\s*(\d{4}/\d{2}/\d{2})',
    ]
    
    # Also try to find just the date pattern YYYY/MM/DD in the text
    ...
    for pattern in patterns:
        ...
        if match:
            ...
            # Convert Persian/Arabic-Indic numerals to regular digits if needed
            persian_digits = '۰۱۲۳۴۵۶۷۸۹'
            arabic_indic_digits = '٠١٢٣٤٥٦٧٨٩'
            regular_digits = '0123456789'
            
            for i, persian in enumerate(persian_digits):
                date_str = date_str.replace(persian, regular_digits[i])
            for i, arabic in enumerate(arabic_indic_digits):
                date_str = date_str.replace(arabic, regular_digits[i])
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python extract_license_expiry_data.py <extracted_json_file> [output_json_file]")
        sys.exit(1)
