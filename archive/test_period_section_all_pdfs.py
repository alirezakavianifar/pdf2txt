"""Test period section extraction on all PDFs."""
...
def process_pdf(pdf_path: Path, output_dir: Path):
    ...
    try:
        ...
        if extracted_json.exists():
            restructure_period_section_json(extracted_json, restructured_json)
            return True
        else:
...
if __name__ == "__main__":
    ...
    # Find all period_section.pdf files
    template_dir = Path("template1")
    pdf_files = list(template_dir.glob("*/period_section.pdf"))
    
    if not pdf_files:
        print("No period_section.pdf files found in template1/")
        ...
        for pdf_name in pdf_names:
            pdf_path = template_dir / pdf_name / "period_section.pdf"
            if pdf_path.exists():
