"""Test power section extraction on all PDFs."""
...
def process_pdf(pdf_path: Path, output_dir: Path):
    ...
    try:
        ...
        if extracted_json.exists():
            restructure_power_section_json(extracted_json, restructured_json)
            return True
        else:
...
if __name__ == "__main__":
    ...
    # Find all power_section.pdf files
    template_dir = Path("template1")
    pdf_files = list(template_dir.glob("*/power_section.pdf"))
    
    if not pdf_files:
        print("No power_section.pdf files found in template1/")
        ...
        for pdf_name in pdf_names:
            pdf_path = template_dir / pdf_name / "power_section.pdf"
            if pdf_path.exists():
