"""Restructure license expiry section for Template 7."""
import json
import re
...
def restructure_license_expiry_template7_json(extracted_json_path, output_json_path):
    """Restructure extracted JSON to include license expiry data for Template 7."""
    ...
    print(f"Restructured license expiry (Template 7) saved to: {output_json_path}")
    if expiry_date:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python restructure_license_expiry_template7.py <extracted_json_file> [output_json_file]")
        sys.exit(1)
    ...
    restructure_license_expiry_template7_json(input_file, output_file)
